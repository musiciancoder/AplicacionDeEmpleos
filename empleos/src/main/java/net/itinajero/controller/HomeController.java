...
public class HomeController {
	...
	@Autowired
	private PasswordEncoder passwordEncoder; // notar que PasswordEncoder no fue definido en clase de servicio, sino en
												// DatabaseWebSecurity mediante @Bean
	...
	public String mostrarListado(Model model) {
		...
		return "listado"; // en spring en el controlado se puede retornar un objeto (html) que no coincide
							// con el tipo definido inicialmente en el metodo
	...
	public String mostrarHome(Model model) {
		...
		/*
		 * ESTE CODIGO LO ELIMINO AL DECLARAR MAS ABAJO EL METODO setGenericos(Model
		 * model) //EJECUCION DE INY DE DEPENDENCIAS CON serviceVacantes List
		 * <Vacante>lista= serviceVacantes.buscarTodas() ;
		 * model.addAttribute("vacantes", lista);
		 */
		return "home"; // nunca se necesita la extension. en spring en el controlado se puede retornar
						// un objeto (html) que no coincide con el tipo definido inicialmente en el
						// metodo
	...
	@GetMapping("/search")
	public String buscar(@ModelAttribute("search") Vacante vacante, Model model) { // Data Binding, con @ModelAttribute
																					// ("search") lo que hay en el
																					// modelo y con Vacante vacante lo q
																					// llega del formulario
		System.out.println("Buscando por: " + vacante);

		// Para q el usuario haga una busqueda por alguna palabra en especifico en el
		// input de la descripcion
		ExampleMatcher matcher = ExampleMatcher.matching().withMatcher("descripcion",
				ExampleMatcher.GenericPropertyMatchers.contains());

		// Example<Vacante> example =Example.of(vacante); //metodo of crea un example,
		// que es un objeto formado con los atributos que no son nulos, es decir los
		// filtros (atributos) que SI hemos seleccionado en la busqueda por filtros
	...
	// METODO PERSONALIZADO QUE SI DETECTA STRING VACIOS AL ENVIAR EL FORMULARIO LOS
	// SETEA A NULL. ESTO ES UTIL PARA BUSCAR SOLO POR UN FILTRO (CATEGORIA POR
	// EJEMPLO), DEJANDO LOS OTROS VACIOS (DESCRIPCION VACIA POR EJEMPLO)
	@InitBinder
	public void initBinder(WebDataBinder binder) {
		binder.registerCustomEditor(String.class, new StringTrimmerEditor(true));
	}

	@ModelAttribute // con esto declaramos atributos al modelo que estaran disponibles y presentes
					// implicitamente en cada uno de los metodos de la clase HomeController
	public void setGenericos(Model model) {
		Vacante vacanteSearch = new Vacante(); // para buscar vacantes de trabajo
		vacanteSearch.reset(); // metodo definido en la clase de modelo Vacante para omitir imagenes en la
								// busqueda
